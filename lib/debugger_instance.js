...
DebuggerInstance.prototype._blackbox = function (event) {
  var should_blackbox;
  if (this.flags.ignore_debugger_statements) {
    var text = event.body.sourceLineText.substr(event.body.sourceColumn, 'debugger'.length);
    console.log(text);
    if (text === 'debugger') {
      should_blackbox = true;
    }
  }
  should_blackbox = should_blackbox || this.breakpoint_info.blackbox_filters.some(function (filter) {
